...
extern lean_object *initialize_leanuweave_Leanuweave(uint8_t builtin);
extern lean_object *uweave_replay_kernel(lean_object *bytes);
...
void shim_uweave_init(void) {
  ...
  lean_object *res = initialize_leanuweave_Leanuweave(1);
...
uint8_t *shim_uweave_replay(const uint8_t *in, size_t len, size_t *out_len) {
  ...
  lean_object *out = uweave_replay_kernel(arr); /* consumes arr */
