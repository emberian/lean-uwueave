//! End-to-end canaries for the mandatory Lean admission-trace certificate.
//!
//! The kind-3 requests used here are emitted by
//! `tests/RuntimeAuthV4AdmissionTraceCorpus.lean`; this crate contains no UWV4
//! request encoder.  Fresh admission, exact retry, and pinned recovery are
//! kept as distinct claims and are checked at their public runtime surfaces.
...
impl TempPath {
    ...
    fn write(label: &str, extension: &str, bytes: &[u8]) -> Self {
        let path = Self::new(label, extension);
        fs::write(&path.0, bytes).expect("write trace-corpus temporary file");
        path
    }
}
...
fn corpus(arguments: &[&str]) -> Vec<u8> {
    let output = Command::new("lake")
        .args([
            "env",
            "lean",
            "--run",
            "tests/RuntimeAuthV4AdmissionTraceCorpus.lean",
        ])
        .args(arguments)
        .current_dir(repo())
        .output()
        .expect("launch Lean admission-trace corpus");
    assert!(
    ...
    assert!(output.stderr.is_empty(), "trace corpus must be byte-only");
    output.stdout
}

fn request(case: &str, signature: &[u8]) -> Vec<u8> {
    let signature = TempPath::write("signature", "bin", signature);
    corpus(&[
        "request",
        case,
        signature
