//! Immutable public-API implementation evidence for debt U-0170.
//!
//! The test connects Lean's canonical logical artifact stream to the shipping
//! checksummed `UWARJ001` journal and exhausts every strict cut of the final
//! outer record. It deliberately does not claim that a filesystem or power
//! loss can produce only these images; that deployment premise remains
//! U-0168.
...
impl TempFile {
    fn new() -> Self {
        ...
        let path = std::env::temp_dir().join(format!(
            "uwueave-debt-u-0170-{}-{nonce}.journal",
            std::process::id()
        ));
...
fn debt_closure_u_0170() {
    ...
    // Every strict final-outer-record cut withholds the incomplete inner
    // logical body. The empty cut is already a complete two-record image.
    ...
    // A corrupt complete middle record is never mistaken for a disposable
    // final prefix, and the valid third record is never skipped into view.
