...
| Persistence and durability | The Rust stores are still in-memory, so there is no shipping persistence implementation. The logical rung now exists in `Durable.lean`: `CanonicalCodec` supplies canonical payload bytes and each frame carries a separate version/domain `FormatTag`; `decodeValue_encodeValue_append` proves framed round-trip with suffix preservation; `decodeFor_tornFrame` and `tornFrame_isProperPrefix` characterize torn frames; `recover_encodeJournal`, `recover_append`, `recover_crashPrefix`, and `recover_crashPrefix_monotone` prove exact and prefix-monotone recovery. `DeploymentAssumptions` merely names `observedPrefix`, `appendOrder`, and `flushDurable` as propositions and constructs no inhabitant. | **OBLIGATION**, with the abstract codec/recovery rung **paid** | Implement a host journal, connect its concrete bytes to `CanonicalCodec`, and prove or differentially validate that the target filesystem and flush protocol satisfy the named assumptions. Nothing here proves filesystem append, rename, or flush atomicity, nor any power-loss guarantee. |
...
| Identity and authenticity | `Authenticity.lean` now names an abstract `SignatureScheme`, domain-separated signed grant/event messages, `AuthenticIssuer`, a trace-relative `EUFStylePremise`, and `ForgeryWitness`. `authenticIssuer_iff_no_received_forgery` and `authenticity_violation_extracts_forgery` give the exact constructive handoff. But the scheme assumes only honest-signature correctness, not security; there is no concrete primitive, byte codec, computational reduction, or integration that admits only authenticated submissions to the shipping gate. | **OBLIGATION** — the abstract handoff is paid; see 3b | Instantiate the model with a concrete signature scheme and canonical signed-byte codec, supply an actual EUF-CMA-style deployment argument, and connect accepted records to gate admission. The gate bounds what a cited grant can **do**; `Byzantine.unauthenticated_submission_can_pass_the_gate` proves that authorization alone does not authenticate who cited it. |
...
| Crash behaviour and recovery | `Durable.lean` proves recovery for its canonical framed journal: exact journals and complete appends recover, a torn final frame is a proper prefix, crash-prefix recovery drops it, and recovered record prefixes are monotone. This is a logical byte-list model only. `DeploymentAssumptions` names prefix observation, append order, and flush durability without constructing or using a filesystem instance; the shipping stores remain in-memory. | **OBLIGATION**, with the logical recovery model **paid** | Bind a concrete host journal and crash model to those propositions and test/prove the refinement. No theorem here grants filesystem atomicity, durability after flush, or recovery after arbitrary corruption; no such deployment claim is made. |
| Byzantine peers | `Byzantine.lean` separates equivocation, forgery, and withholding. `fork_evidence_permanent` and `fork_evidence_iconfluent` make admitted fork evidence permanent under `Gossip`, while `fork_evidence_attributes_author` requires `SignatureAuthentic`; `forged_branch_can_frame_without_authentication` shows why. `unauthenticated_submission_can_pass_the_gate` separates authentication from authorization. `not_quiesced_iff_withheld` and `concrete_withholding` characterize delivery failure. Under `Settled`, grounded announcements, `IdAuthentic`, and issuance, `authentic_issuance_preserves_finality` proves ERA safety; its contrapositive and forged-id witness expose the authenticity boundary. `authenticity_and_delivery_are_independent` proves neither premise subsumes the other. | **OBLIGATION**, with this safety decomposition **paid** | Instantiate `SignatureAuthentic`, `IdAuthentic`, and grounded announcements at the deployed admission layer, then model adversarial transport and a BFT protocol. These theorems do not prove a cryptosystem, Byzantine consensus, censorship resistance, or liveness; withholding remains a separate delivery failure. |
