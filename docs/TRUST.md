...
| Rust `unsafe` (U-0166) | The source currently contains **21 unsafe blocks**: 18 in `rust/src/ffi.rs` and 3 in the direct-shim kernel benchmark. `ffi.rs` also declares the one `unsafe fn`, `take_shim_bytes`. These source-token counts are mechanically checked; local `SAFETY` arguments and inventory are useful controls, not an FFI proof, and the benchmark calls disprove the former concentration claim. | **OBLIGATION** | Remove direct unsafe shim calls from example and benchmark targets; then enforce unsafe only at the dedicated FFI boundary with `unsafe_op_in_unsafe_fn` and an exact AST inventory. Miri cannot follow execution across this FFI boundary, so it cannot close the row. |
