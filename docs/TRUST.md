...
| Persistence and durability | The crate ships three pure-Rust host journals, but not a filesystem theorem. `ArtifactJournal` stores each unchanged canonical V2 frame inside checksummed `UWARJ001` records and delegates bounded semantic admission to the exported Lean validator. `DocumentJournal` stores the unauthenticated typed `MoveLog` subset plus prefix-equal checkpoints in `UWDJRN01`. `HistoryJournal` stores explicit 32-byte event IDs, strictly ordered duplicate-free parent sets, and opaque payloads in `UWHIST01`; parents must already be accepted, exact retries are idempotent, and self-parent, missing-parent, malformed, and same-id/different-event cases refuse. Every accepted history prefix is therefore causally closed at this host layer, but IDs are not authenticated. `BufferedHistoryJournal` optionally holds missing-parent events in a visible finite **volatile** buffer, detects retries/collisions across both stores, and drains ready events in deterministic ID order; reopening intentionally loses pending events because only ready events enter `RawJournal`. Lean's separate `HistoryRuntime.DeliveryState` proves capacity preservation and finite draining, while `PersistentHistoryRuntime.deliverySchema` treats arrival records—including buffered ones—as authoritative and replayable. There is deliberately no refinement between that model and the volatile Rust endpoint. The shared `RawJournal` supplies bounded record lengths, sequence-addressed retry, advisory locking, BLAKE3 checksums, explicit sync policies, poisoning after uncertain I/O, and narrow torn-tail truncation. | **OBLIGATION**, with canonical artifact bytes, the narrow Lean validator, and the three pure-Rust journal/test rungs **paid** | Prove or differentially validate the Rust physical formats and replay adapters against their Lean contracts, then state a filesystem/crash model. If buffered arrivals must survive restart, add a durable arrival queue with its own authority and recovery contract before claiming refinement to `deliverySchema`. Reconcile Lean's duplicate-free parent lists with Rust's stricter increasing canonical order. Bound total file/record counts; harden symlink/path replacement, regular-file and permission policy; specify cross-platform locking and directory-handle/sync behavior; add short-write, open/parent-sync failure, and real crash testing; and add authenticated append plus an externally committed head to detect coherent rewriting or clean-suffix rollback. Bind `HistoryEventId` to a concrete authenticated or content-addressed canonical event encoding. BLAKE3 here is unkeyed corruption detection, not authenticity. The stores are one-record-at-a-time, not an `AtomicBatchObservation` implementation, and no `sync_*` return proves stable media or power-loss survival. |
...
root-level `main` declarations never enter one proof aggregate. The full
Wave-26 Cargo gate has **142 passing Rust tests**. That is valuable
...
`preo_export_v3` corpus measures **6.732 MiB/item**, above the 4 MiB/item cap.
...
| Identity and authenticity | `Authenticity.lean` names an abstract `SignatureScheme`, domain-separated signed grant/event messages, `AuthenticIssuer`, a trace-relative `EUFStylePremise`, and `ForgeryWitness`; `authenticIssuer_iff_no_received_forgery` and `authenticity_violation_extracts_forgery` give the constructive handoff. `AuthenticatedAdmission.authenticIssuer_to_signatureAuthentic` and `AuthenticatedGatedOp.ofAuthenticIssuer` then transport accepted model records through attribution, holder binding, and `Gated.gatedOps`. `Preo.DerivedProgram` now carries each typed expression result with the exact caller-supplied source and exact `Expr.Hole` child path, and its verified materializer requires an external `SourceAuthenticity` proof for every present world. This prevents the adapter from silently inventing positions or laundering a source function into authenticity; it does not supply signatures, causal ownership, or peer identity. Cycle 22 adds the more deployment-shaped but still non-shipping `RuntimeAuthV4` foundation: `signingBytesV4_injective` binds canonical signed-move fields; `decodeBounded` and `validateShape` delimit parsing; `Verified`, `ResolverBoundary`, and `ReadyForExecution` keep verification, stable-id resolution, authority, and membership explicit; `replica_is_signed_issuer` removes an independent asserted actor; and `StorageReceipt`, `LayeredOutcome`, and `WellLayered` keep storage failure distinct. It is deliberately not wired into FORMAT v3, Rust, the C shim, a key registry, or a signature implementation. | **OBLIGATION** — the abstract handoff, typed attribution, and canonical v4 foundation are paid; see 3b | Choose a concrete signature scheme and canonical deployed key/history representation, supply an actual EUF-CMA argument, implement the v4 verifier/resolver/authority/membership/storage pipeline, and only then project with `RuntimeAuthV4.toExecOp` into the existing execution semantics. Connect `SourceAuthenticity` to that deployed admission evidence instead of treating a caller-authored relation as self-authenticating. `VerificationBoundary.Accepts`, `StorageBoundary.Appended`, and `StorageBoundary.DurableObservation` are premises, not deployed mechanisms. The current Rust `DocumentJournal` remains explicitly unauthenticated, and `Byzantine.unauthenticated_submission_can_pass_the_gate` still shows why authorization alone cannot authenticate a submitter. |
