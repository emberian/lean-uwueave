...
**Twelve semantic rows: three PREMISE, six OBLIGATION, one SCOPE, and two PAID
boundaries.** U-0160 is a separate P0 registry-integrity umbrella, not a
thirteenth execution boundary and not semantic closure; it stays open until a
machine-readable gate preserves this exact mapping and rejects stale ledger
claims. Canonical request encoding and build freshness are closed controls.
They do not prove code generation, the external toolchain/runtime premises,
the shim, ABI, unsafe calls, storage glue, or canonical host durability.
