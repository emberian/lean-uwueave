...
| Serialized bytes ⟷ the proved abstract state | Partly proved, and the proved part is real: `replay` is *definitionally* decode → `absReplay` → encode; `replay_encodeRequest`, `decodeBase_encodeRequest`, `decodeOps_encodeRequest`, `decodeGrants_encodeRequest`, and `decodeRevs_encodeRequest` close the input codec against the canonical encoder; `decode_encode_id` and `getWord_encodeView` close the output codec at word level. Separately, `Preo.ProjectionV1.validate` checks schema, explicit resource bounds, stable-ID uniqueness, and projection-visible references before the private `ValidatedProjectionV1` reaches `renderRustSource`; `validateAndRender` is a validated **data-only** transport/rendering boundary. | **OBLIGATION**, partly paid | ⚠ The abstract-to-concrete *symbol* bridge (`Move.lean` §3, `absReplay_matches_miniInterp`) holds on the **2-node universe only** — its own header says so. Generalizing that bridge past two nodes, and giving the seq and era kernels the codec closure the move kernel has, is the named work. `ProjectionV1` neither reconstructs a proof-carrying artifact nor proves that rendered Rust compiles, denotes the source semantics, authorizes an operation, or issues a permit. |
...
| The Lean runtime (`libleanshared`) | Reference counting, the allocator, `lean_alloc_sarray`, `lean_dec_ref`, module initializers. Linked as a dylib from the toolchain, with an rpath baked in by `build.rs`. The ownership contracts are now written at the touched Rust and C calls, but the runtime does not check them. | **OBLIGATION** | Verify the manual reference-count and allocation discipline across the boundary, especially construction of the four typed arrays for `uwueave_encode_request` and the rule that exported kernels consume their Lean array arguments. Written safety contracts make this a specifiable task; they are not a proof of the runtime or its use. |
| `shim.c` | 231 lines, seven exported shim functions (`init`, `encode_request`, `replay`, `seq`, `era`, `request_canonical`, `free`). The new path flattens typed records into Lean arrays, calls the Lean encoder, copies its result, and manages Lean references and `malloc` ownership. It does **not** decide FORMAT-v3 magic, counts, block order, endianness, or bytes. | **OBLIGATION**, and the cheapest one here | Specify and verify the object construction, bounds, ownership, and copying with VST or Frama-C, or generate the shim. Its surface is still small; moving the wire-format decision into Lean narrowed the obligation but did not verify this adapter. |
| The ABI and the FFI boundary | Handwritten `extern "C"` declarations in `rust/src/ffi.rs` still match `shim.c` by convention. Typed `#[repr(C)]` Rust inputs have matching C structs, C `_Static_assert`s pin their size and field offsets, a real-ABI test exercises the route, and every touched call now has an explicit safety contract. Those checks do not prove that the Rust and C declarations agree. | **OBLIGATION** | Generate both declarations from one header or run bindgen, then verify the typed-array conversion against the Lean export signature. Static assertions detect some C layout drift; they cannot establish cross-language ABI agreement or runtime ownership. |
| Rust `unsafe` | **13 textual occurrences, all of them in `rust/src/ffi.rs`** — no `unsafe` anywhere else in the crate. Every touched call and raw-slice/free step now carries a local `SAFETY` argument. That concentration and documentation are paid controls, not an FFI proof. | **OBLIGATION** | Keep the concentration (any new `unsafe` outside `ffi.rs` is a regression) and verify the C/Lean obligations those comments rely on. Miri cannot follow execution across this FFI boundary, so it cannot close the row. |
...
| Cargo/Lean build freshness | **Paid in four independently visible parts:** `Uwueave.lean` imports both `Uwueave.SeqKernel` and `Uwueave.EraKernel`; `rust/build.rs` panics whenever `lake build` fails instead of accepting C left by an earlier build; it compiles emitted submodules recursively; and its `cargo:rerun-if-changed` set names `Uwueave/`, root `Uwueave.lean`, `lakefile.toml`, `lean-toolchain`, `lake-manifest.json`, and `shim.c`. A change to any source, root import, dependency manifest, toolchain pin, or shim therefore invalidates Cargo's cached build-script result, and a failed Lean rebuild cannot proceed to stale C. | **PAID** | Keep these inputs explicit and regression-test the emitted directives when the build layout changes. Any new root-level Lean build input must join the rerun set; this is maintenance of a closed control, not an open trust assumption. |
| Persistence and durability | The Rust stores are still in-memory, so there is no shipping persistence implementation. The reusable logical rung in `Durable.lean` provides canonical payloads, version/domain-tagged frames, exact suffix-preserving round-trip, torn-frame refusal, and exact/prefix-monotone journal recovery. `Preo.ArtifactDurable` now instantiates it as `artifactCodec : CanonicalCodec ArtifactEncoding`: `projectionBytes` carries the explicit artifact format tag; `decodeProjection_projectionBytes_append` proves framed round-trip with trailing bytes; `Examples.noncanonical_naturals_are_refused`, `Examples.overlong_artifact_refused`, and `Examples.wrong_projection_tags_are_refused` reject noncanonical payloads and wrong version/domain tags; `Examples.two_frames_then_torn_third` recovers and decodes exactly two complete artifact encodings before a logically torn third. `DeploymentAssumptions` still only names `observedPrefix`, `appendOrder`, and `flushDurable` as propositions and constructs no inhabitant. | **OBLIGATION**, with the canonical artifact-byte/logical recovery rung **paid** | Implement a host serializer and journal, prove its bytes agree exactly with `ArtifactDurable.projectionBytes`, and prove or differentially validate the filesystem/flush crash refinement. Nothing here proves host serialization, filesystem append or rename atomicity, sector behavior, durability after flush, or any power-loss guarantee. |
...
storage glue, or a future durable host binding.
...
| Identity and authenticity | `Authenticity.lean` names an abstract `SignatureScheme`, domain-separated signed grant/event messages, `AuthenticIssuer`, a trace-relative `EUFStylePremise`, and `ForgeryWitness`; `authenticIssuer_iff_no_received_forgery` and `authenticity_violation_extracts_forgery` give the constructive handoff. The next abstract transport is now paid too: `AuthenticatedAdmission.authenticIssuer_to_signatureAuthentic` turns a trace of accepted signed event records plus `AuthenticIssuer` into the attribution premise used by fork evidence, and `AuthenticatedGatedOp.ofAuthenticIssuer` packages an accepted signed `MoveClaim` with genuine issuance, explicit grant-holder binding, and `Gated.gatedOps`. These are model-level accepted-event/signed-move transports, not deployed admission. | **OBLIGATION** — the abstract handoff through attribution, holder binding, and the gate is paid; see 3b | Choose a concrete signature scheme and canonical signed-byte codec, supply an actual EUF-CMA argument, and refine deployed FORMAT-v3 requests through `Exec`, the generated C path, and the FFI into these accepted records and signed moves. None of those shipping links is proved. `Byzantine.unauthenticated_submission_can_pass_the_gate` still shows why the authorization gate alone cannot authenticate its submitter. |
...
| Crash behaviour and recovery | `Durable.lean` proves recovery for canonical framed journals: exact journals and complete appends recover, a torn final frame is a proper prefix, crash-prefix recovery drops it, and recovered record prefixes are monotone. `Preo.ArtifactDurable.Examples.two_frames_then_torn_third` specializes that logical theorem to canonical, version/domain-tagged `ArtifactEncoding` payloads and proves that two complete frames followed by its explicit `TornFrame` witness recover and decode to exactly the first two encodings; its tag and noncanonical-payload refusal tests delimit accepted bytes. This remains a `List UInt8` crash-prefix model. `DeploymentAssumptions` names prefix observation, append order, and flush durability without constructing a filesystem instance; the shipping stores remain in-memory. | **OBLIGATION**, with canonical artifact bytes and logical torn-tail recovery **paid** | Bind a concrete host serializer, journal, and crash model to `projectionBytes`, `TornFrame`, and the deployment propositions, then test/prove the refinement. No theorem here grants host-codec agreement, filesystem or flush atomicity, durability after flush, arbitrary-corruption recovery, or behavior under actual power loss. |
...
There is no absent-component row now: durability and authentication have
logical rungs, while their deployment bindings remain obligations.
