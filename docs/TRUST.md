...
`classifyFinite` remains total. Two `#guard_msgs` canaries require both
`classify` and `verdict` to expose the refusal. These are controls on an
unverified elaborator: every successful route still has to emit a kernel-checked
term, and no cap or diagnostic makes the metaprogram a theorem.
