...
91 Lean module files under `Uwueave/`, including the nested `Preo` and
...
| `Uwueave/Protocol.lean` | A deep protocol AST for operations, sequence, parallelism, nonempty finite choice, bounded repetition, and synchronization, retaining all seven scheduling axes. Its semantics carries one global strategy pointwise through composition; `elaborate` returns a checked `Session`, `Schedule`, `Plan`, and `UpperBound`. `no_ast_crossing_count_determines_least_meetings` transports Scheduling's exact separation into syntax, so elaboration does not manufacture a meeting count from crossings. There is deliberately no parser in this module. |
...
| `Uwueave/Preo/Export.lean` | The declaration-level one-way export seam: a private `DeclarationBundle` consumes checked fields, `Spec.Verdict`s, world-indexed future certificates, and `Protocol.Elaboration`s, then projects one neutral artifact and canonical encoding. `Examples.whole_export_roundtrips` exercises every exported list and the wire round-trip. Decoded verdict tags remain first-order data; no wire-to-proof constructor is exposed. |
...
| `Uwueave/Authenticity.lean` | The constructive cryptographic handoff without a cryptographic hardness theorem. Domain-separated signing messages bind protocol version, issuer, key epoch, and every grant/event field. `authenticity_violation_extracts_forgery` turns any accepted unissued record into an exact forgery witness; `accepted_antitone_revocation` and `rotation_rejects_older_record` cover revocation and current-key policy. An intentionally insecure toy scheme makes the vocabulary both satisfiable and refutable; deployments still owe an actual EUF-style argument. |
...
table currently holds 349 rows:
