...
123 Lean module files under `Uwueave/`, including the nested `Preo` and
...
| `Uwueave/StatusEffects.lean` | Six-status effects are downsets of an explicit refinement relation, with real lattice operations and exact embedding of the legacy three flags. `infer_is_least` computes the least effect over an explicit finite reach. `TotalSoundEvaluator6` constrains all six cells and is inhabited by `statusOf`; its proof now consumes `ResultStatus.statusOf_eq_iff`, the shared exact inversion API, rather than duplicating six branches. The closed-fork-as-open fixture satisfies the old partial contract but is rejected by the total one. Resolution remains an explicit query constructor, not a hidden Boolean. |
...
table currently holds 477 rows:
