...
114 Lean module files under `Uwueave/`, including the nested `Preo` and
...
| `Uwueave/Preo/Elab.lean` | The command elaborator for checked declaration, certificate, budget, and export surfaces. In addition to ordinary verdict/seam/derive rows, `typed derive` now compiles `Expr.Raw` through `Raw.infer` into an existentially typed `Expr.Program`, exact holes/reads and conservative certificates; generates proof-tied cache/update operations; constructs the equality-future `ResultProgram`; and exposes only reports whose environment is a member of the authored finite reach. A failed inference emits none of those artifacts. `preo_export` remains a proof-indexed, V2-validated, durable data-only boundary. |
...
| `Uwueave/Preo/Expr.lean` | The intrinsically typed first-order expression language now consumed by `typed derive`. `Raw.infer`/`compile?` are the fail-closed existential typechecker, `Program` retains the raw spelling and inference equality, `Term.holes`/`reads` are exact dependencies, and `Term.eval_ext` proves them sufficient. `MergeSafe.sound` and `MonotoneSafe.sound` erase conservative certificates to semantic laws. Ill-typed raw syntax and raw custom nodes are refused; a custom typed node still requires explicit locality/algebraic evidence. |
...
| `Uwueave/Preo/ResultProgram.lean` | Checked six-status programs and reports. A `CheckedDeclaration` fixes its future and explicit resolution in the type, retains a finite reach, infers its least effect, and carries total soundness for all six cells. `CheckedReport` ties output to the actual evaluated site/status and to exact future, resolution, surface, visibility, and disclosure identities. The generic and concrete refusal theorems make false site/policy claims uninhabitable; pixel salience and truth outside the authored reach are not inferred. |
...
table currently holds 466 rows:
