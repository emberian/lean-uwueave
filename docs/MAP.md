...
107 Lean module files under `Uwueave/`, including the nested `Preo` and
...
| `Uwueave/ScheduleSynthesis.lean` | Proof-carrying finite schedule search over a caller-supplied catalog of real `Plan`s. `SearchResult.bound_exists_of_isFound` recovers a catalog member and full five-currency `ProfileUpperBound`; `exhaustive_of_not_isFound` returns a named-currency violation for every supplied candidate. `selectLeast` is least only under an explicit caller `OrderPolicy` and catalog order: `selection_is_policy_dependent` makes two policies select different retained profiles. No catalog generator or arbitrary schedule enumeration is claimed. |
...
| `Uwueave/SeamColoring.lean` | The safety clause of segmentation is exactly graph colouring (`safetyClause_iff_properColoring`); full segmentation additionally needs fiber stability, and `coloring_alone_does_not_segment` proves that residual is real. Over a covering finite pool and finite palette, `minimumSeam_search_total` exhaustively returns either a least certified seam or a proof that none exists; `pin_minimum_is_exact` computes the uniqueness clash's optimum as two colours. No global completeness is claimed without those finite coverage hypotheses. |
...
| `Uwueave/Preo/Demo.lean` | Executable acceptance tests for `preo`: `LoomDoc2` retains seam/cross/derive facets; `TwinQuota` derives a product document seam; `NestedSurface` finds two seams through eight fields and absorbs six checked FREE rows; `KeyedDoc` re-derives the hand bookmarks verdict. `groupedCarrierSurface_state_is_weaveDoc` and `groupedCarrierSurface_core_seed_is_core₀` prove an explicit-seed custom field reaches the hand grouped carrier. `coalescedProfileBudget_is_hand_witness` accepts one exact five-currency profile witness, while crossing counts and meeting floors do not substitute for it. `SemanticExport` then exercises the whole manifest value: exact literal IDs and rows, one budget-bearing checked bundle, canonical roundtrip, format-v2 durable bytes, V2 validation, and deterministic rendering all reduce successfully. The general algebra also reconstructs `weaveDocSeamVerdict` as the same value. |
| `Uwueave/Preo/Elab.lean` | The command elaborator that turns a `preo` declaration into ordinary state/projection definitions plus kernel-checked `Verdict`, `SegVerdict`, `Fourth.Correct`, or `Obligation` values. It supports keyed `per` carriers, application `custom` carriers only with an explicit seed and existing `MergeState`, conservative clash self-seams, two seam rows anywhere in a right-nested document, and checked FREE-row absorption. Standalone `preo_certificate` retains a complete dependent certificate type; `preo_budget` consumes a real five-currency `ProfileUpperBound`. `preo_export` folds explicit manifest rows immediately through proof-indexed builders, emits canonical durable bytes, and requires V2 validation before exposing its private validated value and data-only Rust. Unsupported predicates, unresolved classifications, wrong certificates/plan equalities, and invalid whole manifests fail at their typed or validation boundary. |
| `Uwueave/Preo/Syntax.lean` | The fragment-2 surface and its small semantic support: six built-in field kinds, explicit-seed `custom` carriers, optional keyed families, one- or two-field invariants, one-field derives, report rows, named certificates, proof-carrying profile budgets, and an explicit checked `preo_export` manifest. `proj_iconfluent` is the uniform join-homomorphic projection lift. There is deliberately no deep expression AST; ordinary Lean terms stay opaque except for which declared fields they mention. |
| `Uwueave/Preo/Expr.lean` | An intrinsically typed first-order expression foundation over heterogeneous schemas: typed field references cannot fail, `Term.holes` retains positional reads, and `Term.eval_ext` proves the computed dependencies sufficient. `MergeSafe.sound` and `MonotoneSafe.sound` turn conservative structural certificates into semantic laws; malformed raw syntax and opaque nodes without explicit algebraic evidence are rejected. The analyses are sound but incomplete, and `Preo.Syntax`/`Preo.Elab` do not yet consume this module. |
...
| `Uwueave/Protocol.lean` | A deep protocol AST for operations, sequence, parallelism, nonempty finite choice, bounded repetition, and synchronization, retaining all seven scheduling axes. Its semantics carries one global strategy pointwise through composition; `elaborate` returns a checked `Session`, `Schedule`, and `Plan`, with both peer-only `UpperBound` and exact five-currency `ProfileUpperBound` projections. `no_ast_crossing_count_determines_least_meetings` transports Scheduling's exact separation into syntax, so elaboration does not manufacture a meeting count from crossings. There is deliberately no parser in this module. |
| `Uwueave/Preo/Artifact.lean` | The neutral first-order export vocabulary. Private checked constructors require real verdict, future, session, plan, and exact-plan `ProfileUpperBound` terms before projecting stable IDs, witnesses, demands, origins, promised limits, and realized five-currency profiles. Decoding produces only first-order data and cannot reconstitute semantic proofs; `ArtifactEncoding.decode_canonicalEncoding` proves the complete structural wire round-trip. This is a transport boundary, not a verifier or a minidregg-specific manifest. |
...
| `Uwueave/Preo/ArtifactDurable.lean` | Canonical `List UInt8` serialization for neutral `ArtifactEncoding`. The explicit artifact format is **v2** because its product includes witnessed five-currency budgets; v1 frames are refused rather than decoded under the new shape. `decodeProjection_projectionBytes_append` proves exact version/domain-framed roundtrip with trailing journal bytes; exact decoding rejects surplus and noncanonical payload bytes, and wrong version/domain tags refuse. `Examples.two_frames_then_torn_third` recovers two complete artifact projections before a logically witnessed torn append. Decoding remains first-order and no filesystem/flush/crash refinement is claimed. |
| `Uwueave/Preo/ProjectionV1.lean` | The exact budget-empty legacy host schema. `validate` first requires `encoding.budgets.isEmpty`, returning `budgetsNotSupported actual` otherwise, then checks resource limits, unique IDs, references, crossing indices, and canonical plan profiles before constructing private `ValidatedProjectionV1`. Only that type enters deterministic data-only `renderRustSource`; naturals remain unbounded decimal strings. V1 is not a compatibility alias for the budget-bearing artifact format and does not verify semantics or issue a permit. |
...
| `Uwueave/WorldContext.lean` | Delivery futures extended with active authority capabilities, a downward-closed causal cut, and a version-history base/head. `delivery_projects` forgets these axes and `delivery_lifts` recovers a projected step only under explicit frozen-context and admission hypotheses. `same_world_axes_hide_context` and `projected_delivery_does_not_lift_without_context` prove equal materialized state, pool, frontier, and epoch do not determine the allowed future. Authentication and automatic origin/version attribution remain external. |
...
| `Uwueave/Preo/Incremental.lean` | Conservative differential evaluation for typed `Preo.Expr` environments. `incremental_correct` makes both paths agree with fresh evaluation; an off-dependency change reuses the proof-tied cache with zero counted root evaluations, while a touched term recomputes exactly once. Opaque custom nodes always miss unless their author supplies an `IncrementalLaw` whose result itself proves correctness. The counter is semantic root evaluations, not allocation or reduction cost. |
| `Uwueave/StatusEffects.lean` | Six-status effects are downsets of an explicit refinement relation, with real lattice operations and exact embedding of the legacy three flags. `infer_is_least` computes the least effect over an explicit finite reach. `TotalSoundEvaluator6` constrains all six cells and is inhabited by `statusOf`; the closed-fork-as-open fixture satisfies the old partial contract but is rejected by the total one. Resolution remains an explicit query constructor, not a hidden Boolean. |
...
| `Uwueave/Preo/ProjectionV2.lean` | The budget-bearing host schema. It reuses V1 on a budget-cleared view, then validates action-list histograms, exact obligation coverage, unique budget IDs, session/plan references, plan-session agreement, canonical limits/profiles, realized-profile equality, and every pointwise limit. `full_export_validates` accepts the proof-originated whole export; named fixtures refuse action-profile lies, uncovered obligations, dangling/mismatched references, duplicate IDs, malformed profiles, and exceeded limits. Its deterministic Rust is still first-order data only, with every `Nat` rendered as decimal text. |
...
table currently holds 433 rows:
...
| `CheckedInvariant.toArtifact` (def) | Preo/Artifact | ∀-general | — |
| `ArtifactEncoding.decode_canonicalEncoding` | Preo/Artifact | ∀-general | — |
...
| `decodeProjection_projectionBytes_append` | Preo/ArtifactDurable | ∀-general | — |
...
| `Examples.artifact_example_validates` | Preo/ProjectionV1 | finite-story | — |
| `Examples.duplicate_field_refused` | Preo/ProjectionV1 | finite-story | — |
| `Examples.noncanonical_profile_refused` | Preo/ProjectionV1 | finite-story | — |
| `Examples.wrong_schema_refused` | Preo/ProjectionV1 | finite-story | — |
...
| `Examples.unbounded_decimal_fixture` | Preo/ProjectionV1 | finite-story | — |
| `Examples.empty_projection_rust_fixture` | Preo/ProjectionV1 | finite-story | — |
...
| `Examples.full_export_validates` | Preo/ProjectionV2 | finite-story | — |
| `Examples.action_profile_lie_refused` | Preo/ProjectionV2 | finite-story | — |
| `Examples.uncovered_obligation_refused` | Preo/ProjectionV2 | finite-story | — |
| `Examples.exceeded_budget_limit_refused` | Preo/ProjectionV2 | finite-story | — |
