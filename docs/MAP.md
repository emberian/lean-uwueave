...
172 Lean module files under `Uwueave/`, including the nested `Preo` and
...
| `Uwueave/StatusEffects.lean` | Six-status effects are downsets of an explicit refinement relation, with real lattice operations and exact embedding of the legacy three flags. `infer_is_least` computes the least effect over an explicit finite reach. `TotalSoundEvaluator6` now requires both empty candidates and openness at `pending`; `TotalSoundEvaluator6.semanticsAt` uniformly returns the exact `Status.Semantics` row for all six constructors. `statusOf_totalSound6` inhabits it through the shared ResultStatus inversion API. The pending-with-candidate and closed-fork-as-open fixtures show why the older partial contract is insufficient. Resolution remains an explicit query constructor, not a hidden Boolean. |
...
| `Uwueave/PersistentHistoryRuntime.lean` | The causal event admission function lifted into `PersistentRuntime`'s authoritative cursor/checkpoint contract. `historySchema_step_eq_some_iff` pins accepted steps exactly to `HistoryRuntime.append`; retry and collision theorems preserve the same event identity boundary; `checkpoint_suffix_replay` equates validated checkpoint-plus-suffix replay with whole-log replay; and `fork_arrival_orders_converge` proves the event-set view agrees across two causally legal sibling orders. The new delivery schema persists authoritative arrival records separately from the materialized/pending state; `DeliveryCursorCoherent` states capacity plus exact arrival = materialized-or-pending membership, and reverse-order replay/checkpoint fixtures settle and converge. This is deliberately **not** a refinement of Rust's new `BufferedHistoryJournal`: its bounded pending map is volatile and lost on reopen, while Lean replays authoritative arrivals; Rust additionally requires strictly increasing canonical parent bytes where Lean assumes only `Nodup`. A durable Rust arrival queue and an explicit refinement remain open. |
...
table currently holds 637 rows:
