...
98 Lean module files under `Uwueave/`, including the nested `Preo` and
...
| `Uwueave/Preo/Artifact.lean` | The neutral first-order export vocabulary. Private checked constructors require real verdict, future, session, and plan terms before projecting stable IDs, witnesses, demands, and origins; decoding produces only first-order data and cannot reconstitute semantic proofs. `ArtifactEncoding.decode_canonicalEncoding` proves the complete structural wire round-trip. This is a transport boundary, not a verifier or a minidregg-specific manifest. |
...
| `Uwueave/Preo/Export.lean` | The declaration-level one-way export seam: a private `DeclarationBundle` consumes checked fields, direct `Spec.Verdict`s or answered `Classification`s, world-indexed future certificates, and `Protocol.Elaboration`s, then projects one neutral artifact and canonical encoding. `addClassification_free_agrees_with_addVerdict` and its seam analogue show classification export immediately becomes the checked verdict path; `unresolved_classification_has_no_export_licence` blocks unresolved rows. Decoded verdict tags remain first-order data; no wire-to-proof constructor is exposed. |
| `Uwueave/Preo/ArtifactDurable.lean` | Canonical `List UInt8` serialization for neutral `ArtifactEncoding`. `decodeProjection_projectionBytes_append` proves exact version/domain-framed roundtrip with trailing journal bytes; exact decoding rejects surplus and noncanonical payload bytes, and wrong version/domain tags refuse. `Examples.two_frames_then_torn_third` recovers two complete artifact projections before a logically witnessed torn append. Decoding remains first-order and no filesystem/flush/crash refinement is claimed. |
| `Uwueave/Preo/ProjectionV1.lean` | A bounded, fail-closed host validation boundary for neutral projections. `validate` checks schema, resource limits, unique IDs, references, crossing indices, and canonical profiles before returning a private `ValidatedProjectionV1`. Only that type enters `renderRustSource`; `renderRustSource_eq_of_encoding_eq` makes output a pure function of validated first-order data and `validateAndRender_error` preserves every refusal. Naturals render as unbounded decimal strings, not machine integers. Generated Rust remains data-only and is not proved to compile, verify semantics, or issue a permit. |
...
table currently holds 403 rows:
