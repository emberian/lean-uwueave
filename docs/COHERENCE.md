...
| Disk → root → gate | There are **170** Lean module files under `Uwueave/`. The proof root directly imports **147** Uwueave modules excluding `Audit`; its transitive closure and the audit gate reach all **168** import-pure library modules. `Preo.ArtifactEmitMain` and `Preo.ArtifactInspectionMain` are intentionally separate executable build boundaries: importing both into one aggregate would collide on root `main`, so `Audit` covers their pure `ArtifactEmit` / `ArtifactInspectionV1` dependencies and Lake compiles each Main separately. The live full gate completed **171 jobs**, reports those **147** root modules covered, and audits **23,138** constants. | **The current-wave assembly defect remains closed without confusing CLI entry points with proof modules.** |
...
| MAP → disk → gate | The MAP file table has **170 unique rows for 170 files**, with no missing or extra module. Its keystone table contains **580** unique `(theorem,module)` rows. The two executable Main rows name separate build boundaries; the remaining library modules are inside the root/gate closure. | **The former 34-row exposure remains closed.** The table is a reading aid, not a per-name trust gate. |
| TRANSPORTS | The ledger has **121** unique numbered rows, including the Wave-25 StateProgram, checked-result/certificate V3, planning, canonical-byte/reopen, and diagnostic-inspection crossings (`45h`–`45l`) as well as the earlier ArtifactEmit, checked-manifest, contextual-compilation, status-effect, temporal, and typed-edit boundaries. | **The current-wave crossings are paid and their failure boundaries are recorded.** |
...
| Performance/resource gate | The serialized typed Preoscript corpus now measures **3.946 MiB incremental peak RSS per item** at N=16, down from 4.713 MiB/item and below the explicit 4 MiB/item pass threshold. The same gate preserves exact generated-name/type/row goldens. | **The current typed scaling gate passes.** It is a measured corpus regression guard, not a general memory-complexity theorem. |
| UNDONE ledger | `scripts/undone-census.sh --check` and the generated ledger agree on **159 markers / 157 blocks / 43 source files**. | **Marker prose and generated inventory are synchronized.** This is work accounting, not proof coverage. |
