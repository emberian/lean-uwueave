...
| Disk → root → gate | There are **181** Lean module files under `Uwueave/`. The proof root directly imports **158** Uwueave modules excluding `Audit`; its transitive closure and the audit gate reach all **179** import-pure library modules. `Preo.ArtifactEmitMain` and `Preo.ArtifactInspectionMain` are intentionally separate executable build boundaries: importing both into one aggregate would collide on root `main`, so `Audit` covers their pure `ArtifactEmit` / `ArtifactInspectionV1` dependencies and Lake compiles each Main separately. The live root/Audit gate completed **182 jobs**, reports those **158** root modules covered, and audits **24,921** constants. | **The current-wave assembly defect remains closed without confusing CLI entry points with proof modules.** |
...
| MAP → disk → gate | The MAP file table has **181 unique rows for 181 files**, with no missing or extra module. Its keystone table contains **686** unique `(theorem,module)` rows. The two executable Main rows name separate build boundaries; the remaining library modules are inside the root/gate closure. | **The former 34-row exposure remains closed.** The table is a reading aid, not a per-name trust gate. |
| TRANSPORTS | The ledger has **129** unique numbered rows. Wave 27 adds the signed typed-position/context crossing, authenticated lawful frontier, checked V4 one-way sidecar, durable arrival/storage≠authentication, and strict capability-consumption boundaries (`45p`–`45t` and `46`) beside the earlier state/program/artifact/history crossings. | **The current-wave crossings are paid and their failure boundaries are recorded.** |
...
| UNDONE ledger | `scripts/undone-census.sh --check` and the generated ledger agree on **156 markers / 154 blocks / 43 source files**. | **Marker prose and generated inventory are synchronized.** This is work accounting, not proof coverage. |
