...
Wave-23 automation/trust/native-closure work, the Wave-24 failure-honest
elaboration, projection, and real-artifact execution work, and the Wave-25
state-program, checked-V3, history-journal, inspection, and coherent Quickstart
work, followed by Wave-26 authenticated-observation/V3-export acceptance,
bounded history delivery, and exact typed-position attribution, then Wave-27
signed world context/frontier progress, one-way V4 sidecars, and durable
arrival recovery, Wave-28's authenticated ERA delivery certificate, and
Wave-29's finite repair-menu and finite-history delivery boundaries. The new
...
|---|---|---|
| Root → gate | `#gate_covers_root` parses `Uwueave.lean` with Lean's own header parser and checks every direct root import is in the gate environment; the intentionally indented `TrustFloor` import is an executable regression. | **The original A.1 defect and the brittle line-scanner follow-up are closed.** |
| Disk → root → gate | There are **185** Lean module files under `Uwueave/` and **186** full-build jobs. The proof root directly imports **162** Uwueave modules excluding `Audit`; its transitive closure and the audit gate reach all **183** import-pure library modules and currently check **25,747** constants. `Preo.ArtifactEmitMain` and `Preo.ArtifactInspectionMain` are intentionally separate executable build boundaries: importing both into one aggregate would collide on root `main`, so `Audit` covers their pure dependencies and Lake compiles each Main separately. | **The current-wave assembly defect remains closed without confusing CLI entry points with proof modules.** |
...
| MAP → disk → gate | The MAP file table has **185 unique rows for 185 files**, with no missing or extra module. Its keystone table contains **731** unique `(theorem,module)` rows. The two executable Main rows name separate build boundaries; the remaining library modules are inside the root/gate closure. | **The former 34-row exposure remains closed.** The table is a reading aid, not a per-name trust gate. |
| TRANSPORTS | The ledger has **135** unique numbered rows. Wave 27 adds the signed typed-position/context crossing, authenticated lawful frontier, checked V4 one-way sidecar, durable arrival/storage≠authentication, and strict capability-consumption boundaries (`45p`–`45t` and `46`); Wave 28 adds `45u`; Wave 29 adds `45v`–`45w`; Wave 30 adds `45x`–`45z` for legacy syntax, context-bound exact projection, and ordered raw-only host admission/recovery. | **All three current-wave crossings and their failure boundaries are recorded.** Row `45z` retains deployment-trait, historical-context, external-pin, cryptographic and filesystem premises; it is not an end-to-end theorem. |
| Runtime persistence | Five distinct physical domains cover canonical artifacts, legacy typed `MoveLog` mutations/checkpoints, causally closed history, durable out-of-order arrival, and the crate-private already-checked authenticated-move record. Every domain has its own marker/codec; the authenticated domain additionally has nonce/operation indexes, a prior-head chain and externally pinned reopen. The raw-only runtime orders projection through append/commit and externally pinned recovery revalidates every stored record. | **Five concrete pure-Rust host rungs and their authenticated composition boundary have landed.** Neither the policy traits nor the external pin, cryptography, filesystem behavior, or host-to-Lean refinement is proved. |
| Checked V3 journey and inspection | `Preo.Quickstart` binds one custom `AppState → Expr.Env` query to an exact named future, exact-world certificate, native protocol, selected plan, witnessed budget, and proof-indexed V3 query/result/certificate rows. It proves bounded V3 validation, canonical reopen, and its stack-safe executable frame equal to `ArtifactV3Durable.projectionBytes`. The host gate writes and byte-reopens the **71,011-byte** frame and **142,022-byte** two-frame logical journal. `ArtifactInspectionV1.inspectFrame` / `inspectJournal` version-dispatch V2/V3 canonical decoders, apply bounded validators, and return deterministic JSON marked `diagnostic-only`; the Rust wrapper unwraps physical artifact records but owns no semantic decoder. | **The checked-language-to-real-byte-to-reopen-to-inspection path is coherent.** Inspection remains diagnostic data, never proof reconstruction or deployment authority; filesystem behavior remains test evidence. |
| Observed transactional V3 export | `ObservedBoundResult.attachAtWorld` requires one caller-supplied `ObservationBoundary.Authentic` proof plus separate running- and authored-reach membership and the exact world-indexed certificate. `preo_export_v3` consumes only that exact observed wrapper and exact checked plan/budget/query/future/world values, checks an explicit command-work cap, and validates stable-ID, order, path-depth, and row/resource rules before publishing one transaction. Result/effect/disclosure/certificate rows are constructed through `ArtifactV3Checked`, not authored. The subprocess matrix has **3 green fixtures / 15 red command cases**; the positive prefix audits **48** constants, while bare reports, lookalikes, wrong indices/values, both cap layers, floor contaminants, declaration leakage, and failed-name reuse are executable refusals. | **The checked journey has a consumer-facing failure-honest command.** Authenticity remains an external premise, and no observation is read from a host process, network, device, or filesystem. |
| Typed source/position attribution | `Holes.evalPositions` and `DerivedDocument.deriveAttributedDoc` preserve exact value/source/static-position membership and commute with world-set union. `Preo.DerivedProgram` takes positions only from exact typed `Expr.Hole` analysis, projects back to the original evidence document, and requires `SourceAuthenticity` at its verified materializer. Wrong source, false path, opaque custom code, and position-free literals have focused refusals. | **Typed attribution no longer erases child paths or silently blesses a source function.** The deployment still supplies source authenticity; no signature, causal ownership, or peer identity is inferred. |
| Authenticated causal context and frontier | `AuthenticatedWorldContext` admits a typed-position event only through accepted signature, trace-relative issuance, exact typed value/source/path, roster, active unconsumed grant, holder binding, causal cut and version reach. Its consumption receipt tombstones a step-level grant set exactly; the concrete `grant_unique` premise deliberately enforces strict one-use tokens and rejects a same-grant multi-candidate batch. `AuthenticatedFrontier` separately binds accepted+issued progress, issuer/source, roster, timestamp, before/after frontiers and exact issued/delivered pools; lawful progress never follows from signature alone. | **The former caller-authored authenticity/context gaps are narrowed without merging independent premises.** Cryptographic hardness, an authenticated host trace, and less restrictive multi-candidate capability accounting remain deployment/design work. |
| Authenticated ERA delivery certificate | `AuthenticatedEraCertificate.Verification` consumes one exact `AuthenticatedProgress` plus an independent `CompleteAnnouncement` tying the same signed event to before/after ERA worlds, cut, issued pool, delivered log and lawful frontier. It derives `Settled`, `settledCert`, delivery-scoped free termination and the role seal. `toReusableCertificate` then deliberately erases the signed record and retains only the exact `eraKey` with semantic acceptance. One positive/five exact-red subprocess fixtures pin the bridge and refuse wrong domain, accepted-but-unissued input, incomplete announcement/frontier and wrong-key reuse; prefix floors audit 118 production and 4 support constants. | **Authentication now reaches ERA's concrete certificate without becoming semantic completeness.** Reuse is exact-key delivery reuse, not re-verification; announcements change the key, and no announcement-future certificate or deployed cryptography is claimed. |
| Bounded authored repair menus | `FiniteRepairMenu` checks a written list bound and strict increasing stable IDs, searches only that checked list, and returns the first applicable authored ID with the exact dependent `Repair`, generated available row, and complete eight-axis `Price`; refusal quantifies over exactly the supplied entries. The acceptance gate has two total positives across both Wave-29 leaves and eight exact reds; the repair cases pin least-ID, full-price, exhaustive, bound, and order behavior. | **One finite authoring boundary is executable and proof-carrying.** Numeric IDs are caller policy, not price order; no global repair enumeration, globally least price, or completeness beyond the listed rows is claimed. |
| Finite history at the delivery edge | `FiniteHistoryDelivery.FiniteGrowth` explicitly carries complete authored versions, stable/injective IDs, exact parent/origin shape, closure, coherence, ancestor selection, and policy generation. `DeliveredGrowth` additionally assumes a permutation replay succeeds, accepted records match, the cursor is coherent, and pending is empty. Only then do causal and reversed schedules transport to identical materialized event sets and `eventSetView`. The external six-version lock fixture exercises repeated proof-indexed merges, exact retry, and collision/self/duplicate-parent refusals. | **Arrival-order independence is proved at the finite event-set observation under explicit replay premises.** Semantic history views still require `SameRecord` plus `HistoryConvergent`/`RecordDetermined`; no delivery success, authentication, bytes, host, Rust, or filesystem refinement is inferred. |
| Checked V4 sidecar | `RuntimeAuthV4Checked` is a one-way builder from exact `ReadyForExecution`, shape, grant and context receipts into neutral rows. `RuntimeAuthV4Durable` frames them under `⟨4,162⟩`; this 369-byte sidecar is explicitly not either `UWV4` request wire. Bounded projection validation covers schema, resources, canonical roster/participants/grant, cited scope, node-index consistency and membership. Decode yields neutral `Manifest`, never the private `CheckedManifest`. | **Checked facts reach bounded canonical data without a proof-from-bytes backdoor.** Digests remain opaque; sidecar decode infers no verifier acceptance, authority oracle, or admission result. |
| Context-bound V4 projection and verifier | `RuntimeAuthV4Kernel` gives request kind 3 its own signed opaque context commitment and canonical kind-4 response. Lean owns five decode, eight shape, three host-width decisions, exact projected signing/request bytes and execution lanes; Rust parses only that response. The pluggable verifier includes one context/document/genesis/issuer/epoch-scoped keyed-BLAKE3 symmetric-MAC profile. | **Canonical admission inputs now reach a concrete verifier boundary without a second UWV4 parser.** `VerificationAcceptance` is a trusted-host attestation, not public-key/EUF-CMA evidence or an authority, membership, nonce, execution, append, durability, context-availability, or anti-rollback decision. |
| Authenticated-move admission and persistence | The separate `AuthenticatedMoveJournal` (`UWAMV401`) stores one crate-private checked kind-3 body with exact request/signing/signature bytes, context/policy versions, exact execution binding, nonce and operation scopes, stable/projection/concrete move lanes, kernel observation and prior head. One domain-separated BLAKE3 chain binds the ordered body—including the execution binding—so its externally pinned head commits to that identity; nonce/operation indexes update only after append. The public runtime accepts only raw kind-3 bytes, fixes document/genesis/context plus that framed concrete topology+grant+revocation digest, requires provider and execution index agreement, then orders verification/classification/context/resolution/authority/membership/concrete Lean preflight/append before in-memory commit. Pinned recovery revalidates the full record. | **Atomic checked-record reservation, ordered host admission, and pin-relative historical recovery have a concrete 10/10-green rung.** Definite storage refusal is distinct from indeterminate I/O. Storage itself performs no authentication or policy checks; the host policies and pin are trusted, an absent external pin cannot detect clean rollback, and no cryptographic/filesystem/stable-media theorem follows. |
| Bounded out-of-order history | Lean `HistoryRuntime.DeliveryState` and `PersistentHistoryRuntime.deliverySchema` retain bounded pending arrivals as authoritative replay data. Rust's `BufferedHistoryJournal` remains volatile, while the new `HistoryArrivalJournal` durably appends every canonical arrival before classification, checkpoints accepted/materialized/pending state, restores pending entries on reopen, and drains deterministically when parents arrive. Four focused recovery/refusal tests and one 369-byte V4 opaque-payload integration pass; the latter deliberately accepts a version-mutated payload under a new event id. | **The concrete durable-arrival rung now exists, and storage≠authentication is executable.** There is still no Lean↔Rust codec/refinement or filesystem theorem; IDs and payloads remain unauthenticated, and Lean/Rust parent canonicality differs. |
| Real V2 artifact bytes | `Preo.ArtifactEmit` selects two real checked encodings and its proved-equal stack-safe implementation produces exact `ArtifactDurable.projectionBytes`; the explicit CLI writes only when invoked. The Rust integration test carries both artifacts through Lean validation, `SyncData` append, close/reopen, exact-byte and BLAKE3 comparison, then rejects mutation, truncation, and a wrong version. | **The fixture-to-real-export execution gap is narrowed.** Importing remains I/O-free, and the test is not a filesystem or stable-media theorem. |
...
Wave 25 adds `PersistentHistoryRuntime`, `Preo.StateProgram`,
`Preo.StateProgramSurface`, `Preo.BoundResult`, `Preo.DerivedProgram`, the V3
artifact/projection family, `Preo.ArtifactInspectionV1`, and
`Preo.Quickstart`. The Quickstart is deliberately one dependent chain rather
than stitched diagnostics: the V3 result and certificate rows cannot be made
without the exact checked query, world, future, certified report, and selected
exact branch. The two CLI `*Main` modules remain outside aggregate imports and
are compiled/run explicitly. The Rust `HistoryJournal` adds causally closed
explicit-id host storage without relabelling event IDs as authenticated.

Wave 26 adds `Preo.ObservedBoundResult` and `Preo.ArtifactV3Surface` to the
public module graph, then strengthens existing typed-attribution, V3-validation,
and history-runtime modules. The observation adapter retains four independent
premises—authenticity, running reach, authored reach, and the exact certificate—
while the V3 command consumes that wrapper transactionally and refuses
structural substitutes. The history layer now states the crucial asymmetry
directly: Lean's replayable arrival cursor and Rust's volatile pending endpoint
have different authority/recovery semantics and no refinement theorem.

Wave 27 adds `AuthenticatedFrontier`, `AuthenticatedWorldContext`, and the
isolated `Preo.RuntimeAuthV4{Data,Checked,Durable,ProjectionCore,Projection,
Fixtures,Examples}` leaves. Authentication, issuance, semantic completeness, causal
context, capability consumption, neutral serialization, and persistence remain
named boundaries rather than one ambient “trusted” bit. The Rust durable-arrival
queue closes the concrete restart gap while deliberately accepting opaque
payloads; this makes the remaining authentication/refinement boundary visible
instead of silently claiming it away.

Wave 28 adds the single `AuthenticatedEraCertificate` leaf. It composes the
already authenticated progress event with an independently proved complete ERA
announcement, then exports only a smaller exact-key reusable delivery
certificate. Authentication and reuse are therefore connected, not conflated.

Wave 29 adds `FiniteRepairMenu` and `FiniteHistoryDelivery`. The former makes
bounded canonical author input and exact dependent output explicit; the latter
makes finite-history completeness and successful settled replay explicit. Both
close useful finite surfaces while retaining the global/search/delivery/runtime
premises as named boundaries.
...
- `RuntimeAuthV4.signingBytesV4_injective`, `decodeBounded`,
  `replica_is_signed_issuer`, and `WellLayered` close the legacy model boundary.
  `RuntimeAuthV4Kernel` separately signs an opaque context commitment and
  exports canonical decode, shape, host-width, and projection decisions. The
  concrete keyed-BLAKE3 profile is a symmetric MAC and trusted host rung, not
  a public-key signature or EUF-CMA proof. Nonce, resolution, authorization,
  membership, execution, append, and durable observation remain later stages.
...
map](MAP.md)** — every module") learns none of this exists. The ledger's
