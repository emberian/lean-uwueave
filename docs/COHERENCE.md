...
| Disk → root → gate | There are **184** Lean module files under `Uwueave/`. The proof root directly imports **161** Uwueave modules excluding `Audit`; its transitive closure and the audit gate reach all **182** import-pure library modules. `Preo.ArtifactEmitMain` and `Preo.ArtifactInspectionMain` are intentionally separate executable build boundaries: importing both into one aggregate would collide on root `main`, so `Audit` covers their pure `ArtifactEmit` / `ArtifactInspectionV1` dependencies and Lake compiles each Main separately. The live root/Audit gate completed **185 jobs**, reports those **161** root modules covered, and audits **25,333** constants. | **The current-wave assembly defect remains closed without confusing CLI entry points with proof modules.** |
...
| MAP → disk → gate | The MAP file table has **184 unique rows for 184 files**, with no missing or extra module. Its keystone table contains **721** unique `(theorem,module)` rows. The two executable Main rows name separate build boundaries; the remaining library modules are inside the root/gate closure. | **The former 34-row exposure remains closed.** The table is a reading aid, not a per-name trust gate. |
| TRANSPORTS | The ledger has **132** unique numbered rows. Wave 27 adds the signed typed-position/context crossing, authenticated lawful frontier, checked V4 one-way sidecar, durable arrival/storage≠authentication, and strict capability-consumption boundaries (`45p`–`45t` and `46`); Wave 28 adds `45u`; Wave 29 adds `45v`–`45w` for bounded authored repair menus and explicitly successful finite-history delivery. | **The current-wave crossings are paid and their failure boundaries are recorded.** |
| Runtime persistence | `ArtifactFrame::new` bounds a frame at `MAX_ARTIFACT_FRAME_BYTES = 1 MiB`, checks its v2 envelope, and asks the exported Lean validator for semantic canonicality before `ArtifactJournal` stores unchanged bytes in `UWARJ001`. `DocumentJournal` stores canonical typed `MoveLog` mutations and prefix-equal checkpoints in `UWDJRN01`. The new `HistoryJournal` stores canonical explicit-id events in `UWHIST01`, requires every parent in the accepted prefix, and refuses self/duplicate/reordered parents, missing parents, and id collisions; exact retries are no-write idempotent. | **Three concrete pure-Rust host rungs have landed.** They are not filesystem theorems, authenticated admission, multi-record atomic commit, or refinements of `PersistentRuntime`. |
...
| Checked V4 sidecar | `RuntimeAuthV4Checked` is a one-way builder from exact `ReadyForExecution`, shape, grant and context receipts into neutral rows. `RuntimeAuthV4Durable` frames them under `⟨4,162⟩`; this 369-byte sidecar is explicitly not the existing `UWV4` signed-request wire. Bounded projection validation covers schema, resources, canonical roster/participants/grant, cited scope, node-index consistency and membership. Decode yields neutral `Manifest`, never the private `CheckedManifest`. The Wave-27 runner has **3 new green / 18 new red** fixtures and delegates unchanged transactional floor/rollback coverage to the **3 green / 15 red-command** V3 suite. | **Checked facts reach bounded canonical data without a proof-from-bytes backdoor.** Digests remain opaque; no verifier, authority oracle, or Rust admission endpoint is inferred. |
...
| Native closure and initialization | The data-free `RuntimeInit` directly imports `Exec`, `SeqKernel`, `EraKernel`, and `Preo.ArtifactJournalKernel` (`Uwueave/RuntimeInit.lean:1-18`). `build.rs` takes Lake's setup description as the sole transitive-closure authority, snapshots and stages the exact native objects, verifies the archive member set and bytes, then rechecks Lake paths, setup, objects, every Lean source, and build configuration (`rust/build.rs:43-150,358-438,575-722`). The shim calls only the RuntimeInit initializer (`rust/shim.c:14-17,54-70`). The native-closure gate remains **13** Lake objects / **659,152 bytes** and a **14**-member archive including the shim / **803,520 bytes** (SHA-256 prefix `29cea783`). The serialized Wave-29 `cargo test --all-targets` gate passed **147/147** tests in **23.98s** (**2.38s** compile time). | Stale, extra, missing, or mixed-generation native objects and initializer drift now fail closed. Code generation, compiler/linker correctness, ABI, ownership, runtime behavior, and filesystem semantics remain execution-TCB obligations. |
