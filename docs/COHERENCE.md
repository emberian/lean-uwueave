...
bounded history delivery, and exact typed-position attribution. The new
...
| Disk → root → gate | There are **172** Lean module files under `Uwueave/`. The proof root directly imports **149** Uwueave modules excluding `Audit`; its transitive closure and the audit gate reach all **170** import-pure library modules. `Preo.ArtifactEmitMain` and `Preo.ArtifactInspectionMain` are intentionally separate executable build boundaries: importing both into one aggregate would collide on root `main`, so `Audit` covers their pure `ArtifactEmit` / `ArtifactInspectionV1` dependencies and Lake compiles each Main separately. The live root/Audit gate completed **173 jobs**, reports those **149** root modules covered, and audits **23,757** constants. | **The current-wave assembly defect remains closed without confusing CLI entry points with proof modules.** |
...
| MAP → disk → gate | The MAP file table has **172 unique rows for 172 files**, with no missing or extra module. Its keystone table contains **637** unique `(theorem,module)` rows. The two executable Main rows name separate build boundaries; the remaining library modules are inside the root/gate closure. | **The former 34-row exposure remains closed.** The table is a reading aid, not a per-name trust gate. |
| TRANSPORTS | The ledger has **124** unique numbered rows, including the Wave-25 StateProgram, checked-result/certificate V3, planning, canonical-byte/reopen, and diagnostic-inspection crossings (`45h`–`45l`), the Wave-26 observation/export, typed-attribution, and bounded-history crossings, and the earlier ArtifactEmit, checked-manifest, contextual-compilation, status-effect, temporal, and typed-edit boundaries. | **The current-wave crossings are paid and their failure boundaries are recorded.** |
...
| Bounded out-of-order history | Lean `HistoryRuntime.DeliveryState` separates materialized events from a finite pending list, proves empty validity, non-growing drain, and capacity preservation through successful `receive`/`receiveAll`, and settles causal and fully reversed six-event criss-cross orders to the same event-set view. `PersistentHistoryRuntime.deliverySchema` makes arrivals authoritative and checkpoint/replayable. Rust's optional `BufferedHistoryJournal` instead keeps pending events in a finite **volatile** map; only ready events reach `HistoryJournal`, deterministic ID-order drain settles the same fixture, and reopen intentionally drops pending events. | **Both bounded endpoints are explicit and tested, but they are not the same persistence semantics.** There is no Lean↔Rust codec/refinement; Rust needs a durable arrival queue before it can refine `deliverySchema`. IDs remain unauthenticated, and Lean's `Nodup` parents are weaker than Rust's canonical increasing order. |
...
| Performance/resource gate | The serialized typed Preoscript corpus measures **3.940 MiB incremental peak RSS per item** at N=16 (`1,252,671,488 B` at N=0; `1,318,780,928 B` at N=16), below the unchanged 4 MiB/item threshold. It uses `-j1`, two warmups, median runs, and a nine-run retry when dispersion exceeded policy. The same gate preserves **88** pinned constants plus exact rows, diagnostics, rollback, and report goldens; Typed16's olean remains **736,048 B**. The separate Wave-26 `preo_export_v3` N=16 corpus measures **6.732 MiB/item** and therefore fails that same ceiling. | **The established typed scaling gate passes; the new V3 command is functionally green but scale-red.** These are serialized measured-corpus regression guards, not general memory-complexity theorems. |
| UNDONE ledger | `scripts/undone-census.sh --check` and the generated ledger agree on **158 markers / 156 blocks / 43 source files**. | **Marker prose and generated inventory are synchronized.** This is work accounting, not proof coverage. |
