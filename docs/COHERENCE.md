...
arrival recovery, followed by Wave-28's authenticated ERA delivery certificate. The new
...
| Disk → root → gate | There are **182** Lean module files under `Uwueave/`. The proof root directly imports **159** Uwueave modules excluding `Audit`; its transitive closure and the audit gate reach all **180** import-pure library modules. `Preo.ArtifactEmitMain` and `Preo.ArtifactInspectionMain` are intentionally separate executable build boundaries: importing both into one aggregate would collide on root `main`, so `Audit` covers their pure `ArtifactEmit` / `ArtifactInspectionV1` dependencies and Lake compiles each Main separately. The live root/Audit gate completed **183 jobs**, reports those **159** root modules covered, and audits **25,041** constants. | **The current-wave assembly defect remains closed without confusing CLI entry points with proof modules.** |
...
| MAP → disk → gate | The MAP file table has **182 unique rows for 182 files**, with no missing or extra module. Its keystone table contains **700** unique `(theorem,module)` rows. The two executable Main rows name separate build boundaries; the remaining library modules are inside the root/gate closure. | **The former 34-row exposure remains closed.** The table is a reading aid, not a per-name trust gate. |
| TRANSPORTS | The ledger has **130** unique numbered rows. Wave 27 adds the signed typed-position/context crossing, authenticated lawful frontier, checked V4 one-way sidecar, durable arrival/storage≠authentication, and strict capability-consumption boundaries (`45p`–`45t` and `46`); Wave 28 adds `45u`, exact authenticated ERA progress plus independent semantic completeness into a record-free exact-key delivery certificate. | **The current-wave crossings are paid and their failure boundaries are recorded.** |
...
| Performance/resource gate | The serialized typed Preoscript corpus measures **3.940 MiB incremental peak RSS per item** at N=16 (`1,252,671,488 B` at N=0; `1,318,780,928 B` at N=16), below the unchanged 4 MiB/item threshold. It uses `-j1`, two warmups, median runs, and a nine-run retry when dispersion exceeded policy. The same gate preserves **88** pinned constants plus exact rows, diagnostics, rollback, and report goldens; Typed16's olean remains **736,048 B**. The final Wave-27 `preo_export_v3` N=16 corpus measures **6.706 MiB/item** and therefore fails that same ceiling. The new authenticated world-context, frontier, and V4 layers add only 36,864 / 84,992 / 28,672 bytes per item in the same corpus. | **The established typed scaling gate passes; the V3 command is functionally green but scale-red.** These are serialized measured-corpus regression guards, not general memory-complexity theorems. |
